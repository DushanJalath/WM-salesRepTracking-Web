...
app.post('/regUser', (req, res) => {
    ...
    db.query(sql, values, (err, result) => {
        if (err) return res.json(err);
        return res.json(generatedPassword);
    })
})
...
app.get('/checkEnteredDate/:id', (req, res) => {
    const value = req.params.id;
    const sql = "SELECT time FROM sales WHERE salesId = ?";

    db.query(sql, value, (err, result) => {
        if (err) {
            return res.json({ Message: "Error" }); // Send an error response
        }

        const latestTimestamp = result[0].time; // Extract the timestamp from the result
        ...
        const isPast7Days = timestampDiff < oneWeeksInMillis;

        // Send a response indicating whether it's past 7 days or not
        return res.json({ isPast7Days });
    });
});
...
app.put('/updateSale/:id', (req, res) => {
    ...
    const sql = "UPDATE sales SET repId = ?, customerId = ?, itemName = ?, qty = ?, paymentMethod = ?, bank = ?, branch = ?, amount = ?, remarks = ? WHERE salesId = ?";
